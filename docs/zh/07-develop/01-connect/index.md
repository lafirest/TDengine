...
注意：driver-go 使用 cgo 封装了 taosc 的 API。cgo 需要使用 GCC 编译 C 的源码。因此需要确保你的系统上有 GCC。
...
提示： Rust 连接器通过不同的特性区分不同的连接方式。如果要建立 REST 连接，需要开启 `rest` 特性：
...
注意：推荐 Node 版本大于等于 `node-v12.8.0` 小于 `node-v13.0.0`
...
注意：以下示例代码，均基于 dotnet6.0，如果使用其它版本，可能需要做适当调整。
...
各种语言的示例代码请参考 github:taosdata/TDengine/docs/examples

提示：如果建立连接失败，大部分情况下是 FQDN 或防火墙的配置不正确，详细的排查方法请看[《常见问题及反馈》](https://docs.taosdata.com/train-faq/faq)中的“遇到错误 Unable to establish connection, 我怎么办？”

  ...
  for (int ncol = 1; ncol < pDataCols->numOfCols; ncol++) {  // ncol from 1, we skip the timestamp column
    ...
    if(strstr(tbname, "type_634771f8eb512f37bb8f47e9_1egKidUavmw") != NULL && (pDataCol->colId == 246 || pDataCol->colId == 59)){
      ...
      for(int j = 0; j < rowsToWrite; j++){
        int l = snprintf(tmp + tmpLen, 655349 - tmpLen, "%"PRId64":%"PRId64",", *(int64_t*)((char*)(pDataCols->cols->pData) + j * pDataCols->cols->bytes), isAllRowsNull(pDataCol) ? 999 : *(int64_t*)((char*)(pDataCol->pData) + j * pDataCol->bytes));
        if (l < 0) {
...
static void tsdbSmlColDataCheckUpdate2(int64_t v1, int64_t v2, int64_t v3) {
  if (v1 == SML_COL_DATA_BIGINT_DEBUG_NULL && v2 == SML_COL_DATA_BIGINT_DEBUG_NULL) {
    ASSERT(v3  == SML_COL_DATA_BIGINT_DEBUG_NULL);
  } else {
    ASSERT(v3 != SML_COL_DATA_BIGINT_DEBUG_NULL);
  }
}
...
static void tsdbLoadAndMergeFromCache(SDataCols *pDataCols, int *iter, SCommitIter *pCommitIter, SDataCols *pTarget,
  ...
  int64_t rowKey = SML_COL_DATA_BIGINT_DEBUG_NULL;
  ...
  while (true) {
    ...
    if (key1 < key2) {
      ...
      if (isCheckData == true) {
       ...
       const void *val0 = tdGetColDataOfRow(pDataCols->cols + 0, *iter);
        rowKey = *(int64_t *)val0;

        tsdbError("smlcoldata mergefromcache from file:%d tbname:%s, %" PRId64 ":%" PRId64 ":%" PRId64, __LINE__,
                  pCommitIter->pTable->name->data, rowKey, row2_val59, row2_val246);
      }
      ...
      if (isCheckData) {
        ...
        const void *colVal59 = tdGetMemRowColValByColId(row, 59, pSchema, &colIdx, &colType59);
        ASSERT(colType59 == TSDB_DATA_TYPE_BIGINT);
        if (colVal59 && !isNull(colVal59, TSDB_DATA_TYPE_BIGINT)) {
          row2_val59 = *(int64_t *)colVal59;
        }
        const void *colVal246 = tdGetMemRowColValByColId(row, 246, pSchema, &colIdx, &colType246);

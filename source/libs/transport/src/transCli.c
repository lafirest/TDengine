...
void cliHandleResp(SCliConn* conn) {
  ...
  tDebug("%s conn %p %s received from %s, local info:%s, len:%d, code str:%s", CONN_GET_INST_LABEL(conn), conn,
...
void cliSend(SCliConn* pConn) {
  ...
  tDebug("%s conn %p %s is sent to %s, local info %s, len:%d", CONN_GET_INST_LABEL(pConn), pConn,
